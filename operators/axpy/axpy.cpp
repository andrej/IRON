...
int main(int argc, const char *argv[])
{
    ...
    cxxopts::Options options("AXPY Test");
    ...
    options.add_options()("help,h",
        ...
        cxxopts::value<std::string>()->default_value("golden_axpy/golden_reference.bin"));
    ...
    for (int i = 0; i < N; i++) {
        if (!test_utils::nearly_equal(*(bufOut + i), (*ref_C)[i], 0.01, 1e-6)) {
            errors++;
            // Print the first 100 mismatches
            if (errors <= 100) {
                std::cout << "Mismatch at index " << i << ": " << "Expected: " << (*ref_C)[i] << ", "
