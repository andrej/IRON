...
int main(int argc, const char *argv[])
{
    ...
    cxxopts::Options options("DEQUANT Test");
    ...
    options.add_options()("help,h",
        ...
        "tile_size,t",
        "the tile size of the transfer in std::bfloat16_t",
        cxxopts::value<int>()->default_value("4096"))("block_size,b",
                                                      "the block size of the transfer in std::bfloat16_t",
    ...
    int tile_size = vm["tile_size"].as<int>();
    int group_size = vm["block_size"].as<int>();
    if ((N % tile_size)) {
        std::cerr << "Length must be a multiple of " << tile_size << "." << std::endl;
        return 1;
    }
    if ((tile_size % group_size)) {
        std::cerr << "Tile size must be a multiple of " << group_size << "." << std::endl;
        return 1;
    }
    // Calculate the total volume of the input in terms of int8 data type, since C++
    // doesn't natively support int4 data types
    // The total volume is the total number of int4 elements, so N / 2 with int8 inputs,
    // and total number of bf16 scalar factors, which is appended after each block,
    // so (N / tile_size) * (tile_size / group_size) * 2 for int8 inputs
    int input_volume = (N / 2) + (N / group_size) * 2;
    ...
    auto bo_inA = xrt::bo(device, input_volume * sizeof(std::uint8_t), XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(3));
    auto bo_out = xrt::bo(device, N * sizeof(std::bfloat16_t), XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(4));

    if (verbosity >= 1)
        std::cout << "Writing data into buffer objects." << std::endl;

    std::uint8_t *bufInA = bo_inA.map<std::uint8_t *>();
    memcpy(bufInA, ref.get<std::uint8_t>("A")->data(), input_volume * sizeof(std::uint8_t));
    ...
    double total_bytes = input_volume * sizeof(std::uint8_t) + N * sizeof(std::bfloat16_t); // input and output
